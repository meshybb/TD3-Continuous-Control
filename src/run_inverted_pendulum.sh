...
#SBATCH --output=logs/inverted_pendulum_%A_%a.out  # %A = Job ID, %a = Array index
#SBATCH --error=logs/inverted_pendulum_%A_%a.err
...
# The --resume flag natively handles checking if a state file exists in train.py
# We explicitly set start_timesteps to 1000 and batch_size to 512 to match the final report
python train.py \
